...
users.getAllUser(request);
 %>
 <c:if test="${result >= 1}">
   <script>
     alert("${worker.wiID}" + "${msg}");
   </script>
 </c:if>
 <c:if test="${result < 1}">
   <script>
...
<div class="container">
		...
		<div class="public-content">
			<div class="public-content-header">
				<h3 style="display: inline-block;">会员列表信息</h3>
 ...
 <!-- items是对应调用了Servlet对应方法中 保存到request中的集合对象的名称 -->
  <caption style="font-size:36px;padding-bottom:2%">员工信息</caption>
  ...
  <td style="width:10%">员工职务</td>
  <td style="width:10%">员工密码</td>
  <td style="width:10%">员工住址</td>
  <td style="width:10%">员工电话</td>
  ...
  <c:forEach items="${users}" var="users">
    <tr>	
      ...
      <td>${users.wiGrade}</td>
      <td>${users.wiPsd}</td>
       <td>${users.wiAddress}</td>
      <td>${users.wiPhone}</td>
      <td>${users.wiBorthday}</td>
    
      <div  class="table-fun">
      <td class="ac"><a href="admin/user/updatework.jsp?suboption=nopage"> 修改</a></td>
      <td class="ac"><a href="admin/user/updateAdminInfo.jsp?suboption=nopage">删除</a></td>
      <div>
    </tr>
  </c:forEach>
   </table>
