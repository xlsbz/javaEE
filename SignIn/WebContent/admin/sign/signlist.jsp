...
<div class="container">
 ...
 <!-- items是对应调用了Servlet对应方法中 保存到request中的集合对象的名称 -->
  ...
  <td style="width:10%">签到ID</td>
  <td style="width:10%">签到编号</td>
  <td style="width:10%">签到地点</td>
  <td style="width:10%">签到时间</td>
  <td style="width:10%">签到备注</td>
  ...
  <c:forEach items="${signins}" var="signins">
    <tr>	
      ...
      <td>${signins.signId}</td>
      <td>${signins.signNumber}</td>
      <td>${signins.signAdress}</td>
      <td>${signins.signTime}</td>
      <td>${signins.signBeizhu}</td>
      <td>${signins.admId}</td>
      <div  class="table-fun">
