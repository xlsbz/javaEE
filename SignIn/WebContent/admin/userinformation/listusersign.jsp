...
<body marginwidth="0" marginheight="0">
 ...
 usersigns.getAllUsersign(request);
 %>
 <c:if test="${result >= 1}">
   <script>
     alert("${signuser.user_id}" + "${msg}");
   </script>
 </c:if>
 <c:if test="${result < 1}">
   <script>
...
<div class="container">
 ...
 <!-- items是对应调用了Servlet对应方法中 保存到request中的集合对象的名称 -->
  ...
  <c:forEach items="${usersigns}" var="usersigns">
    <tr>	

      <td>${usersigns.userId}</td>
      <td>${usersigns.userName}</td>
      <td>${usersigns.userDeparent}</td>
      <td>${usersigns.userAdress}</td>
      <td>${usersigns.userTime}</td>
      <td>${usersigns.userState}</td>
  ...
  <paging:paging 
  href="admin/userinformation/listusersign.jsp" 
