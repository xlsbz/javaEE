...
<body marginwidth="0" marginheight="0">
 <%ApplyinformationS applyinformation = new ApplyinformationS();
 
 applyinformation.getAllApplyinformation(request);
 %>
 <c:if test="${result >= 1}">
   <script>
     alert("${signinformation.contId}" + "${msg}");
   </script>
 </c:if>
 <c:if test="${result < 1}">
   <script>
 ...
 <!-- items是对应调用了Servlet对应方法中 保存到request中的集合对象的名称 -->
  ...
  <c:forEach items="${applyinformations}" var="applyinformations">
    <tr>	
      ...
      <td>${applyinformations.contId}</td>
      <td>${applyinformations.contName}</td>
      <td>${applyinformations.contSex}</td>
      <td>${applyinformations.contNumber}</td>
      <td>${applyinformations.contCard}</td>
      <td>${applyinformations.contSchool}</td>
      <td>${applyinformations.contClass}</td>
      <td>${applyinformations.contPhone}</td>
      <td>${applyinformations.contBeizhu}</td>
      <td>${applyinformations.applyId}</td>
      <div  class="table-fun">
  ...
  <paging:paging 
  href="admin/userinformation/listuserinformation.jsp" 
